<template>
  ...
  <div class="frame">
    <div class="page-frame">
      ...
      <p>
        Dobrodošli u svijet životinja! Ovdje možete pronaći informacije o
        različitim vrstama.
      </p>
    </div>
    ...
    <div class="page-frame">
      ...
      <form v-if="showForm" @submit.prevent="addAnimal" class="animal-form">
        ...
        <input
          v-model="newAnimal.nameOfYoung"
          placeholder="Ime mladunčeta"
          required
        />
        <input v-model="newAnimal.imageUrl" placeholder="URL slike" required />
        <input
          v-model="newAnimal.averageLifespanInMinutes"
          type="number"
          placeholder="Životni vijek (u minutama)"
          required
        />
        <input
          v-model="newAnimal.biggestThreat"
          placeholder="Najveće prijetnje"
          required
        />
        <input
          v-model="newAnimal.continents"
          placeholder="Kontinenti (razdvojeni zarezom)"
          required
        />
      ...
      <div v-if="animals.length === 0 && !errorMessage">
        Nema dostupnih životinja.
      </div>
    </div>
      ...
      <ul class="animal-list">
        ...
        >
          ...
          <p>
            ...
            {{ formatLifespan(animal.averageLifespanInMinutes) }}
          </p>
          <p><strong>Najveće prijetnje:</strong> {{ animal.biggestThreat }}</p>
          <p>
            <strong>Kontinenti:</strong>
            {{ formatContinents(animal.continents) }}
          </p>
    ...
    <div class="page-frame last-section">
      <img
        src="@/assets/illustrations/cute-animals-unblast.png"
        alt="Ekološka svijest"
        class="illustration"
      />
    </div>
  </div>
    ...
    async addAnimal() {
      ...
      try {
        const response = await fetch('http://localhost:3000/api/v1/animals', {
          ...
          body: JSON.stringify({
            ...this.newAnimal,
            averageLifespanInMinutes: parseInt(
              this.newAnimal.averageLifespanInMinutes
            ),
            continents: this.newAnimal.continents
              .split(',')
              .map((c) => c.trim()),
          }),
        });
